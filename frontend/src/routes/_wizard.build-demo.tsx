...
const COUNTRIES: {
  ...
  { code: "AE", name: "United Arab Emirates", dial: "+971", flag: "🇦🇪", len: 9, sample: "501234567" },
...
const lenOk = (len: number | number[], n: number) => (Array.isArray(len) ? len.includes(n) : n === len);
...
function TextField({ label, icon: Icon, error, value, id, ...props }: FieldProps) {
  ...
  return (
    <div className="w-full">
      ...
      {error && (
        <p id={errorId} className="mt-1.5 flex items-center gap-1.5 text-xs font-semibold text-destructive">
  ...
  return (
    <div className="w-full">
      ...
      <div className="mt-1.5 flex items-center justify-between gap-2">
        {error ? (
          <p id={errorId} className="flex items-center gap-1.5 text-xs font-semibold text-destructive">
            <AlertCircle className="h-3.5 w-3.5 shrink-0" />
            {error}
          </p>
        ) : (
...
function SectionLabel({ step, title, hint }: { step: number; title: string; hint?: string }) {
  return (
    <div className="flex items-baseline gap-3">
      ...
      <div>
        <h2 className="text-lg font-bold tracking-tight text-foreground font-sans leading-tight">{title}</h2>
...
function PhoneField({ country, digits, error, disabled, onCountry, onDigits, onKeyUp, onBlur }: PhoneFieldProps) {
  ...
  return (
    <div className="w-full">
      <label htmlFor="field-phone" className="mb-2 block text-[13px] font-sans uppercase tracking-wider text-foreground/80 font-bold">
        Phone number *
      </label>
      <div
      ...
      >
        <div ref={dropdownRef} className="relative flex items-center border-r-2 border-border bg-secondary/40 rounded-l-[10px]">
          ...
          >
            ...
            />
            <span>{c.dial}</span>
            <ChevronDown aria-hidden="true" className={cn("h-4 w-4 text-foreground/50 transition-transform duration-200", isOpen && "rotate-180")} />
          </button>
              ...
              {COUNTRIES.map((o) => (
                <button
                  ...
                  className={cn(
                    ...
                    o.code === country && "bg-primary/10 text-primary font-semibold hover:bg-primary/15"
                  )}
                >
                  <img
                    ...
                    loading="lazy"
                  />
                  <span className="font-mono text-xs text-foreground/55 min-w-[26px]">{o.code}</span>
                  <span className="font-medium text-foreground flex-1 truncate">{o.name}</span>
                  <span className="font-semibold text-foreground/80 text-xs shrink-0">{o.dial}</span>
                </button>
              ))}
            </div>
          )}
...
type FormErrors = Partial<Record<"name" | "email" | "company" | "phone" | "industry" | "customIndustry" | "problem_text", string>>;
...
function BuildDemoPage() {
  ...
  const validate = (): FormErrors => {
    ...
    else if (!EMAIL_RE.test(formData.email.trim())) next.email = "That doesn't look like a valid email.";
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    setErrors(found);
    setTouched({ name: true, email: true, company: true, phone: true, industry: true, customIndustry: true, problem_text: true });
    ...
    if (Object.keys(found).length > 0) {
      ...
      const el = formRef.current?.querySelector<HTMLElement>(`[aria-invalid="true"], #field-${firstKey}`);
      el?.focus();
      return;
    }
    ...
    >
      ...
      <div className="w-full max-w-5xl mx-auto z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[210px_1fr] gap-6 lg:gap-8 items-start">
          ...
          <aside className="hidden lg:block" aria-label="Form sections">
            <div className="lg:sticky lg:top-24 rounded-2xl border-2 border-border/60 bg-card/70 backdrop-blur-sm p-4">
              ...
              <ol className="relative space-y-0.5">
                <span aria-hidden="true" className="absolute left-4 top-5 bottom-5 w-px bg-border" />
                      ...
                      >
                        ...
                        <span
                          className={cn(
                            ...
                            active ? "text-primary" : done ? "text-foreground/85" : "text-foreground/55",
                          )}
                        >
                          {label}
                        </span>
          ...
          {/* ── Form card ── */}
          <motion.div
          {...(prefersReduced ? {} : { initial: { opacity: 0, y: 20 }, animate: { opacity: 1, y: 0 }, transition: { duration: 0.5, ease: [0.22, 1, 0.36, 1] } })}
        ...
        >
          ...
          <div className="mb-9">
            ...
            <h1 className="text-3xl sm:text-4xl font-bold text-foreground tracking-tight font-sans hero-text-shadow leading-[1.1]">
          ...
          <form ref={formRef} onSubmit={handleFormSubmit} className="space-y-9" noValidate>
            ...
            >
              <SectionLabel step={1} title="About you" hint="So we know who's testing the agent." />
            ...
            <motion.section
              ...
              className="space-y-4 scroll-mt-28"
            >
              <SectionLabel step={2} title="Your business" hint="This shapes how your agent speaks and what it handles." />
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <TextField
                  id="field-company"
                  label="Company name *"
              ...
              <div className="space-y-2.5">
                  ...
                  {INDUSTRIES.map((ind) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-[13px] font-bold leading-tight font-sans">{ind.label}</span>
                      </button>
                    );
                  })}
                </div>
                ...
                {formData.industry === "other" && (
                  <motion.div
                    {...(prefersReduced ? {} : { initial: { opacity: 0, height: 0 }, animate: { opacity: 1, height: "auto" } })}
                    className="pt-1"
                  >
                    <TextField
            ...
            <motion.section
              ...
              className="space-y-4 scroll-mt-28"
            >
              <SectionLabel step={3} title="What you'd like to solve" hint="The clearer this is, the sharper your demo agent." />
              <TextArea
            ...
            <motion.section
              ...
              className="space-y-4 scroll-mt-28"
            >
              <SectionLabel step={4} title="Pick a voice" hint="You can hear it on your live call." />
                    ...
                    >
                      <span
                        className={cn(
                          ...
                          selected ? "bg-primary/20 text-primary" : "bg-secondary text-foreground/55",
                        )}
                      >
                        <Mic className="h-[18px] w-[18px]" />
                      </span>
                      <span className={cn("text-[15px] font-bold font-sans", selected ? "text-primary" : "text-foreground/90")}>
                        {v.label}
                      </span>
                      {selected && <Check className="ml-auto h-[18px] w-[18px] text-primary shrink-0" />}
                    </button>
                  );
                })}
              </div>
