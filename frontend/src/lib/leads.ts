...
export const submitLead = createServerFn({ method: "POST" })
  .validator(
    z.object({
      ...
      missed_calls: z.number().optional(),
      booking_value: z.number().optional(),
      booking_requirements: z.array(z.string()).optional(),
    }),
  )
  .handler(async ({ data }) => {
    try {
      ...
      const summaryText =
        ...
        `*Weekly Missed Calls:* ${missed_calls !== undefined ? missed_calls : "N/A"}\n` +
        `*Avg Booking Value:* $${booking_value !== undefined ? booking_value : "N/A"}\n` +
        `*Required Details:* ${booking_requirements ? booking_requirements.join(", ") : "N/A"}\n` +
        `*Problem statement:*\n> ${problem_text}`;
