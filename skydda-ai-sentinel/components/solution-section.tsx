      ...
      <div className="max-w-7xl w-full px-6 md:px-12 lg:px-16 space-y-12">
        ...
        <div className="flex flex-col gap-4 max-w-[560px]">
          <div className="flex items-center gap-3 px-4 py-2 border border-zinc-700 w-fit">
            ...
            <span className="text-sm font-medium text-zinc-400 tracking-wide">The Solution</span>
          </div>
          ...
          <p className="text-balance text-zinc-400 text-base leading-relaxed">
            The world's first generative defense engine. Unlike static rule sets, Aegis learns the
            "heartbeat" of your organization. It autonomously distinguishes between an anomaly and
            an attack, creating a secure gateway that allows business to flow while incinerating
            threats instantly.
          </p>
        </div>
            ...
            <div className="absolute bottom-4 left-4 right-4 h-1 flex gap-2">
              {features.map((_, idx) => (
                <div key={idx} className="h-full flex-1 bg-white/10 overflow-hidden">
                  ...
                  {idx < activeIndex && <div className="h-full w-full bg-amber-500/80" />}
                </div>
              ))}
            </div>
          </div>
